...
def load_and_filter(filter_data, nproc, input_data_file,re_load,tokenized_dataset):
    if re_load:
...
# load model to GPU
def load_model(model_type, num_classes, model_directory,device='cuda'):
...
def forward_pass_single_cell(model, example_cell, layer_to_quant,device):
    ...
    for possible_state in possible_states:
        ...
        for i in range(0, total_batch_length, forward_batch_size):
            ...
            input_data_minibatch = pad_tensor_list(input_data_minibatch, 
                                                   ...
                                                   pad_token_id, 
                                                   model_input_size)
            attention_mask = gen_attention_mask(state_minibatch, device, max_len)
...
def quant_cos_sims(model, 
    ...
    def compute_batch_embeddings(minibatch, _max_len = None):
        ...
        attention_mask = gen_attention_mask(minibatch, device, max_len)
    ...
    for i in range(0, total_batch_length, forward_batch_size):
        ...
        if cell_states_to_model is None:
            ...
            if perturb_group == False:
                ...
                original_minibatch_lengths = torch.tensor(original_minibatch["length"], device)
...
def cos_sim_shift(original_emb, 
    ...
    if not perturb_group:
        ...
        if original_minibatch_lengths is not None:
            original_emb = mean_nonpadding_embs(original_emb, original_minibatch_lengths,device)
        # else:
        #     original_emb = torch.mean(original_emb,dim=1,keepdim=True)
    ...
    if minibatch_lengths is not None:
        perturb_emb = mean_nonpadding_embs(minibatch_emb, minibatch_lengths,device)
    else:
...
def gen_attention_mask(minibatch_encoding, device, max_len = None,):
    if max_len == None:
...
class InSilicoPerturber:
    valid_option_dict = {
        ...
        "nproc": {int},
        "device": {torch.device, str, type(None)}  # 添加 device 键
    }
    def __init__(
        self,
        perturb_type="delete",
        ...
        device=None  # 添加 device 参数，默认值为 None
    ):
        """
        ...
        self.nproc = nproc
        self.device = device  # 将 device 参数赋值给 self.device
    ...
    def validate_options(self):
        ...
        if self.device is None:
            ...
            raise ValueError(f"Invalid device type: {type(self.device)}. Must be str or torch.device.")

        # 然后验证其他选项
        for attr_name, valid_options in self.valid_option_dict.items():
            attr_value = getattr(self, attr_name)
            if attr_name == 'device':
                continue  # 已经处理过 device
            if type(attr_value) not in {list, dict} and attr_value in valid_options:
                continue
            valid_type = False
            for option in valid_options:
                if isinstance(option, type) and isinstance(attr_value, option):
                    valid_type = True
                    break
                elif attr_value == option:
                    valid_type = True
                    break
            if not valid_type:
                error_msg = (
    ...
    def in_silico_perturb(self,
        ...
        if self.perturb_group == False:
            ...
            for i in trange(len(filtered_input_data)):
                ...
                original_emb = forward_pass_single_cell(model, example_cell, layer_to_quant,device)
