    ...
    for i in trange(0, total_batch_length, forward_batch_size):
        ...
        with torch.no_grad():
            outputs = model(
                ...
                attention_mask=gen_attention_mask(minibatch,device),
            )
...
class EmbExtractor:
    valid_option_dict = {
        ...
        "device": {torch.device, str, type(None)}
    }
    ...
    def extract_embs(
        ...
        if tokenized_dataset is None:
            ...
            re_load = False
        print('re_load:',re_load)

        filtered_input_data = load_and_filter(
            self.filter_data, self.nproc, input_data_file,re_load,tokenized_dataset=tokenized_dataset
        )
        # downsampled_data = downsample_and_sort(filtered_input_data, self.max_ncells)
        downsampled_data = filtered_input_data
        model = load_model(self.model_type, self.num_classes, model_directory,self.device)
        ...
        embs_array = embs_df.values

        # 指定输出路径
        output_path = Path(output_directory) / (output_prefix + ".npy")

        # 保存为 .npy 文件
        np.save(output_path, embs_array)
