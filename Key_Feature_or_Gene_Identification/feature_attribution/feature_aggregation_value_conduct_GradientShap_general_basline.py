# -*- coding: utf-8 -*-
"""
Feature Attribution Analysis using GradientShap for Deep Neural Networks
This script performs feature importance analysis using GradientShap method
...
class CaptumShapCalculatorCPU:
    """
    SHAP Calculator using Captum's GradientShap for CPU computation
    """
    ...
    def _compute_shap_values_define(
        ...
        -----------
        target_type: int
            Target index for model output or other usable target;
            For multi-output models, specify the output channel/dimension of interest.
        baseline_type: str, options ['zero', 'mean', 'random_mean', 'random', 'mix']
            - 'zero':        Use zero vector as baseline
            - 'mean':        Use mean of X_input_tensor as baseline
            - 'random_mean': Random sample from X_input_tensor and take mean as baseline
            - 'random':      Use multiple random samples as baselines
            - 'mix':         Weighted combination of mean and random baselines using alpha
        random_baseline_size: int
            Number of samples for random sampling in 'random_mean', 'random' or 'mix' baseline types
        alpha: float
        ...
        all_deltas: np.ndarray
            Convergence indicator array from captum, same length as number of samples
        """
        ...
        if baseline_type == 'zero':
            ...
            # Randomly select random_baseline_size samples from [0, N) range
            ...
            # Don't take mean, directly use random_samples as multiple baselines
            baseline_full = random_samples  # shape: (random_baseline_size, C, H, W)
        ...
        # Batch computation of SHAP values
        for batch_idx in tqdm(range(num_batches), desc="Computing SHAP Values"):
            ...
            if baseline_type == 'random':
                # For multiple baselines, directly pass the entire baseline_dist
                baseline_batch = baseline_full
            else:
                # For single baseline, pass the baseline corresponding to current batch
            ...
            # Take average of delta values for each sample, resulting in shape (batch_size, )
    ...
    def compute_shap(self, type_class, baseline_type='zero',
            ...
            alpha=0.5,
            stdevs=0.09):
        """Compute SHAP values for all class types"""
    ...
    def shap_save(self, save_path):
        """Save SHAP values and approximation errors to CSV files"""
...
parser.add_argument("--baseline_type", type=str, choices=["zero", "mean", "random_mean","random", "mix"], default="random",
        help="Baseline type: 'zero', 'mean', 'random_mean', 'random', or 'mix'. Default is 'random'.")
...
# Start GradientShap analysis for DNN model
...
print('run feature_aggregation_value_conduct_DNN_GradientShap.py success, feature aggregation value and prediction outcome are deployed in ./output')
