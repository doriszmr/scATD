...
#
# # 2. 应用DBSCAN聚类
# """eps和min_samples根据数据和需要调整,
# detail in /home/luozeyu/desktop/VAE_pretraining/code/uamp_plot_test_for_data_quality.py"""
# dbscan = DBSCAN(eps=0.5, min_samples=10)
# labels = dbscan.fit_predict(scRNAseq_matrix_original)
#
# # 3. 分析聚类结果
# n_clusters = len(set(labels)) - (1 if -1 in labels else 0)  # 不包括离群值
# print('Number of Clusters: ', n_clusters)
#
# cluster_info = {'Number of Clusters': n_clusters}
#
# # 4. 保存聚类簇个数信息到Excel
# df = pd.DataFrame([cluster_info])
# excel_path = os.path.join(save_path, 'cluster_info.xlsx')
# df.to_excel(excel_path, index=False)
#
# # 5. 过滤离群值并保存筛选后的数据
# scRNAseq_matrix = scRNAseq_matrix_original[labels != -1]
# npy_path = os.path.join(save_path, 'scRNAseq_filtered_data.npy')
# np.save(npy_path, scRNAseq_matrix)
##不使用DBSCAN去离群值的情况
